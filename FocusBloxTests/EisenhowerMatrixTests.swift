...
final class EisenhowerMatrixTests: XCTestCase {
    ...
    func createTask(title: String, importance: Int, urgency: String, isCompleted: Bool = false) -> LocalTask {
        let task = LocalTask(
            title: title,
    ...
    func test_doFirstQuadrant_filtersUrgentAndHighPriority() async throws {
        ...
        let doFirst1 = createTask(title: "Urgent Important 1", importance: 3, urgency: "urgent")
        let doFirst2 = createTask(title: "Urgent Important 2", importance: 3, urgency: "urgent")

        // Other quadrants
        let schedule = createTask(title: "Not Urgent Important", importance: 3, urgency: "not_urgent")
        let delegate = createTask(title: "Urgent Less Important", importance: 2, urgency: "urgent")
        let eliminate = createTask(title: "Not Urgent Less Important", importance: 1, urgency: "not_urgent")
        ...
        let doFirstTasks = planItems.filter { $0.urgency == "urgent" && $0.importance == 3 && !$0.isCompleted }
    ...
    func test_doFirstQuadrant_excludesCompletedTasks() async throws {
        ...
        let active = createTask(title: "Active Urgent", importance: 3, urgency: "urgent")
        let completed = createTask(title: "Completed Urgent", importance: 3, urgency: "urgent", isCompleted: true)
    ...
    func test_scheduleQuadrant_filtersNotUrgentAndHighPriority() async throws {
        ...
        let schedule1 = createTask(title: "Plan This 1", importance: 3, urgency: "not_urgent")
        let schedule2 = createTask(title: "Plan This 2", importance: 3, urgency: "not_urgent")

        // Other quadrants
        let doFirst = createTask(title: "Urgent Important", importance: 3, urgency: "urgent")
        let delegate = createTask(title: "Urgent Less Important", importance: 2, urgency: "urgent")
        ...
        let planItems = try await syncEngine.sync()
        let scheduleTasks = planItems.filter { $0.urgency == "not_urgent" && $0.importance == 3 && !$0.isCompleted }
    ...
    func test_delegateQuadrant_filtersUrgentAndLowerPriority() async throws {
        ...
        let delegate1 = createTask(title: "Urgent Priority 2", importance: 2, urgency: "urgent")
        let delegate2 = createTask(title: "Urgent Priority 1", importance: 1, urgency: "urgent")
        let delegate3 = createTask(title: "Urgent Priority 0", importance: 0, urgency: "urgent")

        // Other quadrants
        let doFirst = createTask(title: "Urgent Priority 3", importance: 3, urgency: "urgent")
        ...
        let planItems = try await syncEngine.sync()
        let delegateTasks = planItems.filter { $0.urgency == "urgent" && ($0.importance ?? 0) < 3 && !$0.isCompleted }
    ...
    func test_eliminateQuadrant_filtersNotUrgentAndLowerPriority() async throws {
        ...
        let eliminate1 = createTask(title: "Low Priority 2", importance: 2, urgency: "not_urgent")
        let eliminate2 = createTask(title: "Low Priority 1", importance: 1, urgency: "not_urgent")
        let eliminate3 = createTask(title: "Low Priority 0", importance: 0, urgency: "not_urgent")

        // Other quadrants
        let schedule = createTask(title: "Not Urgent Priority 3", importance: 3, urgency: "not_urgent")
        ...
        let planItems = try await syncEngine.sync()
        let eliminateTasks = planItems.filter { $0.urgency == "not_urgent" && ($0.importance ?? 0) < 3 && !$0.isCompleted }
    ...
    func test_allQuadrants_distributeTasksCorrectly() async throws {
        ...
        // 1 task per quadrant
        let doFirst = createTask(title: "Q1: Do First", importance: 3, urgency: "urgent")
        let schedule = createTask(title: "Q2: Schedule", importance: 3, urgency: "not_urgent")
        let delegate = createTask(title: "Q3: Delegate", importance: 2, urgency: "urgent")
        let eliminate = createTask(title: "Q4: Eliminate", importance: 1, urgency: "not_urgent")
        ...
        let q1 = planItems.filter { $0.urgency == "urgent" && $0.importance == 3 && !$0.isCompleted }
        let q2 = planItems.filter { $0.urgency == "not_urgent" && $0.importance == 3 && !$0.isCompleted }
        let q3 = planItems.filter { $0.urgency == "urgent" && ($0.importance ?? 0) < 3 && !$0.isCompleted }
        let q4 = planItems.filter { $0.urgency == "not_urgent" && ($0.importance ?? 0) < 3 && !$0.isCompleted }
    ...
    func test_allTasksCompleted_allQuadrantsEmpty() async throws {
        ...
        // All tasks completed
        let t1 = createTask(title: "Completed 1", importance: 3, urgency: "urgent", isCompleted: true)
        let t2 = createTask(title: "Completed 2", importance: 3, urgency: "not_urgent", isCompleted: true)
        let t3 = createTask(title: "Completed 3", importance: 2, urgency: "urgent", isCompleted: true)
        let t4 = createTask(title: "Completed 4", importance: 1, urgency: "not_urgent", isCompleted: true)

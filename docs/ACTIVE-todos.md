...
| BUG_122 | Sort-Lock: Nicht alle Task-Attribute editierbar waehrend Sortier-Sperre | High | S | Beide | Wenn ein Task fuer Sortierung gesperrt ist (`sortLocked`), soll er weiterhin editierbar sein — das ist der Zweck der Sperre. Manche Attribute lassen sich aendern, andere nicht. Alle Attribute pruefen und freigeben. |
